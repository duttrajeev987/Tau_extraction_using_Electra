        ...
        if ~isempty(taus_POP(i,b).x) && size(taus_POP(i,b).x, 3) >= 1
            ...
            if size(tau_3D, 1) >= EF_index && size(tau_3D, 2) >= T_index
                ...
                if any(positive_mask)
                    ...
                    tau_band_avg_simple = mean(tau_k_positive);
...
title('ZrNiSn POP Scattering Rates - EF=0, T=300K', 'FontSize', 14);
...
title('ZrNiSn POP Relaxation Times - EF=0, T=300K', 'FontSize', 14);
...
baseName = 'ZrNiSn_POP_300K';

% Create output directory
outputDir = 'POP_xmgrace_300K_full';
...
fprintf(fid, 'title "ZrNiSn POP Scattering Rates - EF=0, T=300K"\n');
...
fprintf(fid, 'title "ZrNiSn POP Relaxation Times - EF=0, T=300K"\n');
fprintf(fid, 'xaxis label "Energy [eV]"\n');
