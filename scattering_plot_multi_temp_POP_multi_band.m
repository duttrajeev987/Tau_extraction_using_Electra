...
fprintf('Processing Screened POP data for EF=0, T=300K\n');
...
title('ZrNiSn Screened POP - EF=0, T=300K, all bands', 'FontSize', 14);
...
baseName = 'ZrNiSn_ScreenedPOP_300K';

% Create output directory
outputDir = 'screenedPOP_xmgrace_300K';
...
for b = 1:numBands
    if h_created(b) && ~isempty(E_avg_by_band{b})
        ...
        fprintf(fid, '# ZrNiSn Screened POP - Band %d\n', b);
...
if ~isempty(E_avg_all)
    ...
    fid = fopen(filename, 'w');
    fprintf(fid, '# ZrNiSn Screened POP - Overall Average (All Bands)\n');
    fprintf(fid, '# EF=0, T=300K\n');
    fprintf(fid, '# Energy (eV)  Scattering Rate (fs^-1)\n');
...
fprintf(fid, '# xmgrace batch file for Screened POP scattering rates\n');
fprintf(fid, '# Generated from MATLAB\n\n');
fprintf(fid, 'title "ZrNiSn Screened POP - EF=0, T=300K"\n');
...
for b = 1:numBands
    if h_created(b)
        ...
        fprintf(fid, '# xmgrace batch file for Screened POP Band %d only\n', b);
        fprintf(fid, 'title "ZrNiSn Screened POP - Band %d, EF=0, T=300K"\n', b);
        fprintf(fid, 'xaxis label "Energy [eV]"\n');
        fprintf(fid, 'yaxis label "Scattering Rate \\xt\\f{Symbol}\\f{Times-Italic}\\f{} \\S-1\\N [fs\\S-1\\N]"\n');
        fprintf(fid, 'yaxis scale logarithmic\n');
        fprintf(fid, 'legend on\n\n');
...
fid = fopen(filename, 'w');
fprintf(fid, '========================================\n');
fprintf(fid, 'ZrNiSn Screened POP Scattering Data\n');
fprintf(fid, '========================================\n');
...
fprintf('========================================\n');
fprintf('Scattering type: Screened POP\n');
