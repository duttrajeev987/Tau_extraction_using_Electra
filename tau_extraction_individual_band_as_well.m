...
% Initialize arrays for data collection - SIX METHODS
...
% Store intermediate quantities for M5 and M6
...
TDF_by_band_M6 = cell(numBands, 1);  % Store individual band TDF for M6
...
if exist('TDF', 'var')
    ...
    fprintf('M5 and M6 will use M4 as approximation.\n');
    use_TDF = false;
end
...
fprintf('M6: TDF-sum tau = sum(TDF_band) / (V^2(E)*g(E)), S = 1/<tau>\n');

%% Main processing loop for M1-M4 and collecting V²·g and TDF for M5/M6
for idx = 1:max_idx
        ...
        if ~isempty(taus_IIS(i,b).x) && size(taus_IIS(i,b).x, 3) >= 1
            ...
            if size(tau_3D, 1) >= EF_index && size(tau_3D, 2) >= T_index
                ...
                if any(positive_mask)
                    ...
                    all_tau = [aInll_tau; tau_k_positive];
                    ...
                    % Calculate V²·g sum for M5 and M6
...
%% METHOD 6: TDF-sum tau = sum(individual_band_TDF(E)) / (V²(E)·g(E))
fprintf('\n=== Calculating M5 and M6 ===\n');
fprintf('M5: TDF-based tau = TDF.xx(E) / (V^2(E)*g(E))\n');
fprintf('M6: TDF-sum tau = sum(TDF_band(E)) / (V^2(E)*g(E))\n');
...
if use_TDF && exist('TDF', 'var') && isfield(TDF, 'xx')
    ...
    % Initialize overall M5 and M6 arrays
    E_avg_all_M5 = zeros(N_overall, 1);
    tau_avg_all_M5 = zeros(N_overall, 1);
    sca_avg_all_M5 = zeros(N_overall, 1);
    
    E_avg_all_M6 = zeros(N_overall, 1);
    tau_avg_all_M6 = zeros(N_overall, 1);
    sca_avg_all_M6 = zeros(N_overall, 1);
    
    % Calculate per-band TDF for M6
    % First, get total TDF at each energy point
    TDF_total_per_E = TDF_xx_extracted;
    
    % Now we need individual band TDF contributions
    % Since TDF is typically proportional to V²*tau*g, 
    % we can estimate individual band TDF as: TDF_band = TDF_total * (V2g_band / V2g_total)
    for b = 1:numBands
        n_band_points = length(E_avg_by_band_V2g{b});
        TDF_by_band_M6{b} = zeros(n_band_points, 1);
        ...
        if n_band_points > 0
            for j = 1:n_band_points
                ...
                % For M6, we need to partition TDF among bands
                % We'll compute the total V2g across all bands at this energy
                total_V2g = 0;
                for bb = 1:numBands
                    if ~isempty(E_avg_by_band_V2g{bb})
                        [~, idx_bb] = min(abs(E_avg_by_band_V2g{bb} - E_val));
                        if idx_bb <= length(V2g_avg_by_band{bb})
                            total_V2g = total_V2g + V2g_avg_by_band{bb}(idx_bb);
                        end
                    end
                end
                
                % Partition TDF proportional to band's contribution to V2g
                if total_V2g > 0
                    TDF_band_val = TDF_at_E * (V2g_val / total_V2g);
                else
                    TDF_band_val = 0;
                end
                
                TDF_by_band_M6{b}(j) = TDF_band_val;
            end
        end
    end
    
    % Per-band M5 and M6 calculation
    for b = 1:numBands
        n_band_points = length(E_avg_by_band_V2g{b});
        ...
        if n_band_points > 0
            ...
            E_avg_by_band_M6{b} = zeros(n_band_points, 1);
            tau_avg_by_band_M6{b} = zeros(n_band_points, 1);
            sca_avg_by_band_M6{b} = zeros(n_band_points, 1);
            ...
            for j = 1:n_band_points
                ...
                % M6: tau_M6 = TDF_band(E) / (V²(E)·g(E)) 
                % where TDF_band is the individual band's TDF
                TDF_band_val = TDF_by_band_M6{b}(j);
                if V2g_val > 0 && TDF_band_val > 0
                    tau_M6_val = TDF_band_val / V2g_val;
                else
                    tau_M6_val = NaN;
                end
                ...
                E_avg_by_band_M6{b}(j) = E_val;
                tau_avg_by_band_M6{b}(j) = tau_M6_val;
                if ~isnan(tau_M6_val) && tau_M6_val > 0
                    sca_avg_by_band_M6{b}(j) = 1 / tau_M6_val;
                else
                    sca_avg_by_band_M6{b}(j) = NaN;
                end
            end
        end
    end
    
    % Overall M5 and M6: Use the same energy points as M1-M4
    for j = 1:N_overall
        E_val = E_avg_all_M1(j);
        
        % Sum V²·g across bands at this energy point
        V2g_total = 0;
        sum_TDF_bands = 0;  % For M6: sum of individual band TDFs
        ...
        for b = 1:numBands
            if ~isempty(E_avg_by_band_V2g{b})
                ...
                if idx_b <= length(V2g_avg_by_band{b})
                    ...
                    if idx_b <= length(TDF_by_band_M6{b})
                        sum_TDF_bands = sum_TDF_bands + TDF_by_band_M6{b}(idx_b);
                    end
                end
            end
        end
        
        % Get total TDF.xx at this energy for M5
        if exist('E_array', 'var') && ~isempty(E_array) && length(E_array) == size(TDF.xx, 1)
        ...
        % M6: tau = sum_TDF_bands / V2g_total
        if V2g_total > 0 && sum_TDF_bands > 0
            tau_M6_overall = sum_TDF_bands / V2g_total;
        else
            tau_M6_overall = NaN;
        end
        ...
        E_avg_all_M6(j) = E_val;
        tau_avg_all_M6(j) = tau_M6_overall;
        if ~isnan(tau_M6_overall) && tau_M6_overall > 0
            sca_avg_all_M6(j) = 1 / tau_M6_overall;
        else
            sca_avg_all_M6(j) = NaN;
        end
    end
    
    % Remove NaN values from M5 and M6 arrays
    ...
    fprintf('M6 calculation complete. %d valid points out of %d.\n', sum(valid_M6), N_overall);
    fprintf('M6/M5 ratio: %.4f\n', nanmean(tau_avg_all_M6_valid ./ tau_avg_all_M5_valid));
else
    fprintf('WARNING: TDF.xx not available. M5 and M6 set equal to M4.\n');
    E_avg_all_M5_valid = E_avg_all_M4;
    tau_avg_all_M5_valid = tau_avg_all_M4;
    sca_avg_all_M5_valid = sca_avg_all_M4;
    
    E_avg_all_M6_valid = E_avg_all_M4;
    tau_avg_all_M6_valid = tau_avg_all_M4;
    sca_avg_all_M6_valid = sca_avg_all_M4;
end
...
if ~isempty(E_avg_all_M1)
    ...
    if exist('E_avg_all_M6_valid', 'var')
        plot(E_avg_all_M6_valid, sca_avg_all_M6_valid, 'c-', 'LineWidth', 3, 'DisplayName', 'M6: 1/<tau>_{TDF-sum}');
    end
end
...
title('ZrNiSn IIS - All Scattering Rate Methods (6 Methods)', 'FontSize', 14);
...
if ~isempty(E_avg_all_M1)
    ...
    if exist('E_avg_all_M6_valid', 'var')
        plot(E_avg_all_M6_valid, tau_avg_all_M6_valid, 'c-', 'LineWidth', 3, 'DisplayName', 'M6: sum(TDF_{band})/(V^2g)');
    end
end
...
title('ZrNiSn IIS - All Relaxation Time Methods (6 Methods)', 'FontSize', 14);
legend('Location', 'best', 'FontSize', 8);
...
methods_names = {'M1: Simple <S>', 'M2: DOS-wt <S>', 'M3: DOS-wt <tau>', ...
                 'M4: Transport (V^2g)', 'M5: TDF/(V^2g)', 'M6: sum(TDF_{band})/(V^2g)'};
...
if exist('tau_avg_all_M6_valid', 'var')
    tau_M6_mean = mean(tau_avg_all_M6_valid);
    fprintf('M6: %.4f fs\n', tau_M6_mean);
    
    fprintf('\n--- Ratios Relative to M5 ---\n');
    % Interpolate M1-M4 and M6 to M5 energy grid for comparison
    ...
    tau_M6_interp = interp1(E_avg_all_M6_valid, tau_avg_all_M6_valid, E_avg_all_M5_valid, 'linear');
    ...
    fprintf('M6/M5: %.4f\n', nanmean(tau_M6_interp ./ tau_avg_all_M5_valid));
end
...
filename = fullfile(outputDir, sprintf('%s_overall_6methods.dat', baseName));
fid = fopen(filename, 'w');
fprintf(fid, '# Complete 6-method comparison\n');
fprintf(fid, '# M5: tau = TDF.xx(E, EF=0, T=300K) / (V^2(E)*g(E))\n');
fprintf(fid, '# M6: tau = sum(TDF_band(E)) / (V^2(E)*g(E))\n');
fprintf(fid, '# Energy  Tau_M1  Tau_M2  Tau_M3  Tau_M4  Tau_M5  Tau_M6\n');
...
if exist('E_avg_all_M5_valid', 'var')
    ...
    for j = 1:n_pts
        ...
        fprintf(fid, '%.6f  %.6e  %.6e  %.6e  %.6e  %.6e  %.6e\n', E_val, tau1, tau2, tau3, tau4, tau5, tau6);
    end
else
    ...
    for j = 1:n_pts
        fprintf(fid, '%.6f  %.6e  %.6e  %.6e  %.6e  %.6e  %.6e\n', ...
                ...
                tau_avg_all_M3(j), tau_avg_all_M4(j), tau_avg_all_M4(j), tau_avg_all_M4(j));
    end
end
fclose(fid);

% Also save per-band TDF data for M6
if use_TDF && exist('E_avg_all_M6_valid', 'var')
    filename_M6 = fullfile(outputDir, sprintf('%s_per_band_TDF_M6.dat', baseName));
    fid_M6 = fopen(filename_M6, 'w');
    fprintf(fid_M6, '# Per-band TDF contributions for M6\n');
    fprintf(fid_M6, '# Energy  TDF_Band1  TDF_Band2  TDF_Band3  TDF_Total  Tau_M6\n');
    
    n_pts = length(E_avg_all_M6_valid);
    for j = 1:n_pts
        E_val = E_avg_all_M6_valid(j);
        
        % Get TDF for each band at this energy
        TDF_b = zeros(1, numBands);
        for b = 1:numBands
            if ~isempty(E_avg_by_band_V2g{b})
                [~, idx_b] = min(abs(E_avg_by_band_V2g{b} - E_val));
                if idx_b <= length(TDF_by_band_M6{b})
                    TDF_b(b) = TDF_by_band_M6{b}(idx_b);
                end
            end
        end
        
        TDF_total = sum(TDF_b);
        tau_M6 = tau_avg_all_M6_valid(j);
        
        fprintf(fid_M6, '%.6f', E_val);
        for b = 1:numBands
            fprintf(fid_M6, '  %.6e', TDF_b(b));
        end
        fprintf(fid_M6, '  %.6e  %.6e\n', TDF_total, tau_M6);
    end
    fclose(fid_M6);
end
