...
figure1 = figure('Name', 'Scattering Rates - Screened POP');
fig1 = gcf;
fig1.Position(3:4) = [1200, 800];
hold on;

figure2 = figure('Name', 'Relaxation Times - Screened POP');
fig2 = gcf;
...
TDF_POP_per_band_M7 = cell(numBands, 1);
...
T_index = 7;             % Temperature index (1st in dim 4 = 300K)
...
% Check TDF_sep structure with POP fields
if exist('TDF_sep', 'var') && isfield(TDF_sep, 'POP') && isfield(TDF_sep.POP, 'xx')
    fprintf('✓ TDF_sep.POP.xx found [%d x %d x %d]\n', size(TDF_sep.POP.xx));
else
    error('TDF_sep.POP.xx not found! Check if TDF_sep structure exists with POP field.');
end

% Check TDF_sep.POP (POP total)
if isfield(TDF_sep.POP, 'xx')
    fprintf('✓ TDF_sep.POP.xx found [%d x %d x %d]\n', size(TDF_sep.POP.xx));
else
    error('TDF_sep.POP.xx not found!');
end

% Check TDF_sep.POP_n (band-resolved POP) - note the different naming convention
if isfield(TDF_sep, 'POP_n') && isfield(TDF_sep.POP_n, 'xx')
    fprintf('✓ TDF_sep.POP_n.xx found [%d x %d x %d x %d]\n', size(TDF_sep.POP_n.xx));
else
    % Alternative: check if band-resolved data exists elsewhere
    if exist('TDF_n', 'var') && isfield(TDF_n, 'POP')
        fprintf('✓ TDF_n.POP found (alternative structure)\n');
    else
        warning('TDF_sep.POP_n.xx not found! Band-resolved POP TDF may not be available.');
        warning('M7 will use the same data as M5 if band-resolved data is unavailable.');
    end
end
...
fprintf('========================================\n');
fprintf('Extracting POP TDF data at EF=%d, T=%d\n', EF_index, T_index);
fprintf('========================================\n');

% M5: Total POP TDF
TDF_POP_total = TDF_sep.POP.xx(:, EF_index, T_index);

fprintf('M5 - Total POP TDF:\n');
fprintf('  POP TDF: [%.4e, %.4e]\n', min(TDF_POP_total(TDF_POP_total>0)), max(TDF_POP_total));
fprintf('  Non-zero points: %d/%d\n\n', sum(TDF_POP_total>0), length(TDF_POP_total));

% M7: Band-resolved POP TDF
TDF_POP_n_bands = cell(numBands, 1);

% Try different possible structures for band-resolved POP TDF
has_band_resolved = false;
if isfield(TDF_sep, 'POP_n') && isfield(TDF_sep.POP_n, 'xx')
    has_band_resolved = true;
    fprintf('Using TDF_sep.POP_n.xx for band-resolved POP TDF\n');
    for b = 1:numBands
        TDF_POP_n_bands{b} = squeeze(TDF_sep.POP_n.xx(:, b, EF_index, T_index));
        
        pos_POP = TDF_POP_n_bands{b}(TDF_POP_n_bands{b} > 0);
        fprintf('M7 - Band %d POP TDF:\n', b);
        fprintf('  POP: [%.4e, %.4e], Non-zero: %d/%d points\n\n', ...
                min(pos_POP), max(pos_POP), sum(TDF_POP_n_bands{b}>0), length(TDF_POP_n_bands{b}));
    end
elseif exist('TDF_n', 'var') && isfield(TDF_n, 'POP')
    has_band_resolved = true;
    fprintf('Using TDF_n.POP for band-resolved POP TDF\n');
    for b = 1:numBands
        TDF_POP_n_bands{b} = squeeze(TDF_n.POP(:, b, EF_index, T_index));
    ...
    warning('Band-resolved POP TDF not found. M7 will use the total TDF for all bands.');
end
...
for idx = 1:max_idx
    ...
    for b = 1:numBands
        % Changed from taus_IIS to taus_POP
...
%% METHOD 5: Total POP TDF
fprintf('\n=== M5: Total POP TDF = TDF_sep.POP.xx ===\n');
...
for j = 1:N_overall
    ...
    % Interpolate TDF_POP_total at this energy
    if exist('E_array', 'var') && length(E_array) == length(TDF_POP_total)
        TDF_POP_at_E = interp1(E_array, TDF_POP_total, E_val, 'linear', 0);
    else
        [~, idx_e] = min(abs(E_array - E_val));
        TDF_POP_at_E = TDF_POP_total(min(idx_e, length(TDF_POP_total)));
    end
    TDF_POP_at_E = max(TDF_POP_at_E, 0);
    
    if V2g_total > 0 && TDF_POP_at_E > 0
        tau_M5 = TDF_POP_at_E / V2g_total;
    else
        tau_M5 = NaN;
    end
...
%% METHOD 7: Band-resolved POP TDF
fprintf('\n=== M7: Band-resolved POP TDF ===\n');
fprintf('    tau = Σ(TDF_POP_n) / Σ(V²·g)\n');
...
for b = 1:numBands
    TDF_POP_per_band_M7{b} = zeros(N_overall, 1);
end
...
for j = 1:N_overall
    ...
    % Sum band-resolved POP TDFs
    TDF_POP_sum = 0;
    for b = 1:numBands
        if has_band_resolved
            if exist('E_array', 'var') && length(E_array) == length(TDF_POP_n_bands{b})
                TDF_POP_val = interp1(E_array, TDF_POP_n_bands{b}, E_val, 'linear', 0);
            else
                [~, idx_e] = min(abs(E_array - E_val));
                if idx_e <= length(TDF_POP_n_bands{b})
                    TDF_POP_val = TDF_POP_n_bands{b}(idx_e);
                else
                    TDF_POP_val = 0;
                end
            end
            TDF_POP_val = max(TDF_POP_val, 0);
        else
            % Fallback: distribute total TDF equally among bands
            if exist('E_array', 'var')
                TDF_POP_val = interp1(E_array, TDF_POP_total, E_val, 'linear', 0) / numBands;
            else
                [~, idx_e] = min(abs(E_array - E_val));
                TDF_POP_val = TDF_POP_total(min(idx_e, length(TDF_POP_total))) / numBands;
            end
        end
        
        TDF_POP_per_band_M7{b}(j) = TDF_POP_val;
        TDF_POP_sum = TDF_POP_sum + TDF_POP_val;
    end
    
    if V2g_total > 0 && TDF_POP_sum > 0
        tau_M7 = TDF_POP_sum / V2g_total;
    else
        tau_M7 = NaN;
    end
...
if ~isempty(E_avg_all_M1)
    ...
    if exist('E_avg_all_M5_valid', 'var')
        plot(E_avg_all_M5_valid, sca_avg_all_M5_valid, 'm-', 'LineWidth', 2.5, 'DisplayName', 'M5: 1/<tau>_{POP-total}');
    end
    if exist('E_avg_all_M6_valid', 'var')
        plot(E_avg_all_M6_valid, sca_avg_all_M6_valid, 'c-', 'LineWidth', 2.5, 'DisplayName', 'M6: 1/<tau>_{POP-calc}');
    end
    if exist('E_avg_all_M7_valid', 'var')
        plot(E_avg_all_M7_valid, sca_avg_all_M7_valid, 'Color', [0.8 0.4 0], 'LineWidth', 3, ...
             'DisplayName', 'M7: 1/<tau>_{POP-bands}');
    end
end
...
title('ZrNiSn - Scattering Rates (Screened POP, 7 Methods)', 'FontSize', 14);
...
if ~isempty(E_avg_all_M1)
    ...
    if exist('E_avg_all_M5_valid', 'var')
        plot(E_avg_all_M5_valid, tau_avg_all_M5_valid, 'm-', 'LineWidth', 2.5, 'DisplayName', 'M5: TDF_{POP-total}/V²g');
    end
    if exist('E_avg_all_M6_valid', 'var')
    ...
    if exist('E_avg_all_M7_valid', 'var')
        plot(E_avg_all_M7_valid, tau_avg_all_M7_valid, 'Color', [0.8 0.4 0], 'LineWidth', 3, ...
             'DisplayName', 'M7: ΣTDF_{POP-bands}/V²g');
    end
end
...
title('ZrNiSn - Relaxation Times (Screened POP, 7 Methods)', 'FontSize', 14);
legend('Location', 'best', 'FontSize', 8);
...
%% SAVE DATA
fprintf('\n=== Saving data ===\n');
baseName = 'ZrNiSn_screenedPOP_900K_7methods';
outputDir = 'screenedPOP_xmgrace_7methods_900K';
...
fprintf(fid, '# 7-method comparison: tau values for screened POP at EF=0, T=300K\n');
fprintf(fid, '# All POP-only for M4-M7\n');
fprintf(fid, '# M4: Transport-wt <tau> = Sigma(tau*V^2*g) / Sigma(V^2*g)\n');
fprintf(fid, '# M5: TDF_POP_total / Sigma(V^2*g)\n');
fprintf(fid, '# M6: Sigma(TDF_band_calc) / Sigma(V^2*g), TDF_band = Sigma(tau_k*V^2_k*g_k)\n');
fprintf(fid, '# M7: Sigma(TDF_POP_n) / Sigma(V^2*g)\n');
...
% Save band-resolved POP TDF data for M7
filename_M7 = fullfile(outputDir, sprintf('%s_band_POP_TDF_M7.dat', baseName));
fid_M7 = fopen(filename_M7, 'w');
fprintf(fid_M7, '# Band-resolved POP TDF for M7\n');
fprintf(fid_M7, '# TDF_POP_n at EF=0, T=300K\n');
fprintf(fid_M7, '# Energy  TDF_POP_B1  TDF_POP_B2  TDF_POP_B3  TDF_POP_Sum  V2g_Total  Tau_M7\n');
...
for j = 1:n_pts
    ...
    TDF_sum = 0;
    for b = 1:numBands
        tdf_val = TDF_POP_per_band_M7{b}(j);
...
% Save M5 POP TDF data
filename_M5 = fullfile(outputDir, sprintf('%s_total_POP_TDF_M5.dat', baseName));
fid_M5 = fopen(filename_M5, 'w');
fprintf(fid_M5, '# Total POP TDF for M5\n');
fprintf(fid_M5, '# TDF_POP at EF=0, T=300K\n');
fprintf(fid_M5, '# Energy  TDF_POP  V2g_Total  Tau_M5\n');

for j = 1:length(E_avg_all_M5_valid)
    E_val = E_avg_all_M5_valid(j);
    
    % Interpolate TDF value
    if exist('E_array', 'var')
        TDF_POP_at_E = interp1(E_array, TDF_POP_total, E_val, 'linear', 0);
    else
        TDF_POP_at_E = 0;
    ...
    fprintf(fid_M5, '%.6f  %.6e  %.6e  %.6e\n', ...
            E_val, TDF_POP_at_E, V2g_total, tau_avg_all_M5_valid(j));
end
fclose(fid_M5);

fprintf('✓ Data saved to: %s\n', outputDir);
fprintf('  - %s_overall.dat (all 7 methods)\n', baseName);
fprintf('  - %s_band_POP_TDF_M7.dat (per-band POP TDF)\n', baseName);
fprintf('  - %s_total_POP_TDF_M5.dat (total POP TDF with components)\n', baseName);
fprintf('\n=== Complete ===\n');
